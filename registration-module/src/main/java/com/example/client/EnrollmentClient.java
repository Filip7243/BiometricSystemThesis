...
public class EnrollmentClient {
    ...
    public static final String ENROLLMENTS_API_URL = "http://localhost:8080/api/v1/enrollments";
    ...
    public List<RoomEntranceDTO> getNumberOfEntrancesToEachRoomOnDate(LocalDate date, Long buildingId) {
        try {
            ...
            String urlString = String.format(
                    ENROLLMENTS_API_URL + "/entrances-to-room?date=%s&buildingId=%d",
                    date,
                    buildingId
            );
    ...
    public List<UnconfirmedEntranceDTO> getUnconfirmedEntrancesPerUserByRoom() {
        try {
            HttpRequest request = createGetRequest(
                    ENROLLMENTS_API_URL + "/unconfirmed-entrances-per-user-by-room"
            );
    ...
    public List<UserEnrollmentConfirmationDTO> getUserEnrollmentConfirmationRate(Long userId) {
        try {
            String urlString = String.format(
                    ENROLLMENTS_API_URL + "/enrollments-confirmation-rate?&userId=%d",
                    userId
            );
    ...
    public List<LateControlDTO> getLateControlByUserAndRoom(LocalDate date, Long userId, int expectedHour) {
        try {
            String urlString = String.format(
                    ENROLLMENTS_API_URL + "/late-control?date=%s&userId=%d&expectedHour=%d",
                    date.toString(),
                    userId,
                    expectedHour
            );
