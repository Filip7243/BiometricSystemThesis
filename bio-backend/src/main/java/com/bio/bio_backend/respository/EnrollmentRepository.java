...
public interface EnrollmentRepository extends JpaRepository<Enrollment, Long> {
    ...
    @Query("SELECT e.room.roomNumber, b.buildingNumber, COUNT(e) " +
            "FROM Enrollment e " +
            "JOIN e.room r " +
            "JOIN r.building b " +
    ...
    List<Object[]> getNumberOfEntrancesToEachRoomOnDate(
            @Param("date") LocalDate date,
            @Param("buildingId") Long buildingId);

    @Query("SELECT e.user.firstName, e.user.lastName, e.room.roomNumber, e.room.building.buildingNumber, COUNT(e) " +
            "FROM Enrollment e " +
    ...
    List<Object[]> getUnconfirmedEntrancesPerUserByRoom();

    @Query("SELECT e.user.firstName, e.isConfirmed, e.fingerprint.fingerType, COUNT(e) " +
            "FROM Enrollment e " +
    ...
    List<Object[]> getUserEnrollmentConfirmationRate(@Param("userId") Long userId);

    @Query("SELECT e.user.firstName, e.user.lastName, e.room.roomNumber, e.room.building.buildingNumber, e.enrollmentDate " +
            "FROM Enrollment e " +
            "WHERE FUNCTION('DATE', e.enrollmentDate) = :date " +
            "AND e.user.id = :userId " +
            "AND HOUR(e.enrollmentDate) > :expectedHour")
    List<Object[]> getLateControlByUserAndRoom(
            @Param("date") LocalDate date,
